...
seqtab.nochim[nrow(seqtab.nochim)+1,] = apply(seqtab.nochim, 2, sum)
abun_sums <- seqtab.nochim[nrow(seqtab.nochim),]
abundance <- as.numeric(abun_sums)

# extract seqeunces
sequence <- colnames(seqtab.nochim)

# save sequences and total abundances as new dataframe
...
physeqob <- phyloseq(tax_table(tax), 
