...
# load taxa data
tax <- read.table(paste0(path_out, "Taxa_Table.txt"), 
                  stringsAsFactors = F)
# rename columns
colnames(tax) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")

# load abundance data
seqtab <- read.table(paste0(path_out, "Seq_Abun_Table.txt"), 
                     stringsAsFactors = F, 
                     header = T)
...
seqtab_names <- merge(plates_fil, seqtab, by.x="barcode", by.y = "row.names")
# rename row names to sample names
rownames(seqtab_names) <- seqtab_names$sample
# remove defunct columns
seqtab_names <- seqtab_names[-c(1,2)]
...
# for Taiwan 2016 only, remove all Bsal
...
# subset taxa to include only the non-empty ASV assignments
...
# transform to matrix
seqtab_names_mat <- as.matrix(seqtab_names_filtered)

tax_mat <- as.matrix(tax) # required to create phyloseq object

require(phyloseq)
dada2 <- phyloseq(tax_table(tax_mat), 
                  otu_table(seqtab_names_mat, taxa_are_rows = FALSE), 
