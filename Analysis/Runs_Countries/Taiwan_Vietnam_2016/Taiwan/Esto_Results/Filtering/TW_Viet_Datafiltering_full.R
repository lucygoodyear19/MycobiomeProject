...
# MiSeq plate plan
setwd("/Documents/CMEECoursework/Project/Data-scripts/Taiwan_Vietnam/Filtering")
...
## (2.2) Remove OTUs with BLAST e-values >e-20
#setwd("C:/Users/jenny/Dropbox/Jen's desktop/Results/sequencing/MiSeq/3-Vietnam_Taiwan/2x300bp/7-filtered_files/pear")
evalue <- read.csv("BLASTevalue-20samp.csv", header=T, stringsAsFactor=FALSE, check.names = F) ##44791 OTUs x 384 samples

## (2.3) Remove OTUs with BLAST identity <85%
#setwd("C:/Users/jenny/Dropbox/Jen's desktop/Results/sequencing/MiSeq/3-Vietnam_Taiwan/2x300bp/7-filtered_files/pear")
identity <- read.csv("BLASTidentity85samp.csv", header=T, stringsAsFactor=FALSE, check.names = F) #44049 OTUs x 384 samples
...
F_only$blast_filter <- "nomatch"
F_only$blast_filter[!is.na(F_only$evalue) & F_only$evalue > 1e-20] <- "e20"
F_only$blast_filter[!is.na(F_only$evalue) & F_only$evalue < 1e-20 & F_only$evalue > 1e-50] <- "e20-e50"
F_only$blast_filter[!is.na(F_only$evalue) & F_only$evalue < 1e-50] <- "e50"
# view breakdown of the blast_filter column
# including sum as a check, which should equal no. of OTUs
addmargins(table(F_only$blast_filter))

# keep only OTUs assigned a BLAST e-value <e-20
F_only20 <- F_only[F_only$blast_filter %in% c("e20-e50","e50"),]
# remove BLAST filter column
F_only20 <- F_only20[,!names(F_only20) %in% "blast_filter"]

# separate out OTU info (first 22 columns) and sample info (all other columns)
info <- F_only20[,1:22]
evalsamptot <- F_only20[,23:ncol(F_only20)]
# remove samples without reads in fungal OTUs
# sum all read columns and save as vector
sampsum <- colSums(evalsamptot)
# filter by sums not equal to zero
evalsamp <- evalsamptot[,sampsum != 0]

# combine with OTU info
evalue <- cbind(info,evalsamp)
#setwd("C:/Users/jenny/Dropbox/Jen's desktop/Results/sequencing/MiSeq/3-Vietnam_Taiwan/2x300bp/7-filtered_files/pear")
write.table(evalue, "BLASTevalue-20samp.csv", sep=",", row.names = F) ##44791 OTUs x 382 samples
...
# remove percentage symbol from BLAST identity column and make numeric
evalue$identnum <- gsub("%", "", evalue$identity)
evalue$identnum <- as.numeric(evalue$identnum)

# view minimum BLAST identity
min(evalue$identnum,na.rm=T)
# view tally of BLAST identities as table and histogram
table(evalue$identnum)
hist(evalue$identnum)

# plot BLAST identity against e-value
boxplot(log10(evalue$evalue) ~ evalue$identnum, xlab="BLAST identity (%)", ylab="log10(BLAST e-value)", las=2)
abline(h=-20, col="red"); abline(h=-50, col="green"); abline(v=80)
# check number of OTUs meeting different BLAST identity thresholds
nrow(evalue[evalue$identnum >= 80,])
nrow(evalue[evalue$identnum >= 85,])
nrow(evalue[evalue$identnum >= 90,])

# remove OTUs with BLAST identity < 85%
evalue85 <- evalue[evalue$identnum >= 85,]
# remove BLAST identity column
evalue85 <- evalue85[,!names(evalue85) %in% "identnum"]
...
# separate out OTU info (first 22 columns) and sample info (all other columns)
info <- evalue85[,1:22]
identsamptot <- evalue85[,23:ncol(evalue85)]
# remove samples without reads in remaining OTUs
# sum all read columns and save as vector
sampsum <- colSums(identsamptot)
# filter by sums not equal to zero
identsamp <- identsamptot[,sampsum != 0]

#combine with OTU info
identity <- cbind(info,identsamp)
#setwd("C:/Users/jenny/Dropbox/Jen's desktop/Results/sequencing/MiSeq/3-Vietnam_Taiwan/2x300bp/7-filtered_files/pear")
write.table(identity, "BLASTidentity85samp.csv", sep=",", row.names = F) #44049 OTUs x 382 samples
...
posc <- cbind(identity$otuid,identity$species,identity[grep("posC",names(identity))])
...
# remove posC OTUs
identityposc <- identity[!identity$otuid %in% poscotu,]

# check which columns are info vs samples
#names(identityposc)

# separate out OTU info (first 22 columns) and sample info (all other columns)
#info <- identityposc[,1:22]
#poscsamptot <- identityposc[,23:ncol(identityposc)]
# remove samples without reads in remaining OTUs
# sum all read columns and save as vector
#sampsum <- colSums(poscsamptot)
# filter by sums not equal to zero
#poscsamp <- poscsamptot[,sampsum != 0]

# combine with OTU info
#poscotutab <- cbind(info,poscsamp)
#setwd("C:/Users/jenny/Dropbox/Jen's desktop/Results/sequencing/MiSeq/3-Vietnam_Taiwan/2x300bp/6-filtered_files/pear")
#write.table(poscotutab, "posc_samp0.csv", sep=",", row.names=F) #38000 OTUs; 273 samples
