...
ifelse(!dir.exists(file.path(paste0(results_path, "/Summary/"))), dir.create(file.path(paste0(results_path, "/Summary/"))), FALSE)
# set directory for results to be sent to
path_out <- paste0(results_path, "/Summary/")
...
# sites <- unique(samps[,c("Country", "Longitude", "Latitude")])
...
find_taxa_breakdown(taxa, seqs_pres_abs_t, samps, "Phylum", "Latitude")
...
find_taxa_breakdown(taxa, seqs_pres_abs_t, samps, "Class", "Latitude")
...
# plot bar charts for the following options
find_taxa_breakdown_abun(taxa, seqs_t, samps, "Phylum", "Latitude")
...
#   if (conts_samps$Latitude[row] %in% c("China", "Kazakhstan", "Mongolia",
...
samps_caudata_countries <- unique(as.character(samps$Latitude[samps$A_Order == "Caudata"]))
samps_subs_order <- samps[samps$Latitude %in% samps_caudata_countries,]
...
# create list of ASVs per Latitude
Latitude_prop_asv <- list()
for (Latitude in samps$Latitude) {
  sub <- find_asvs_for_venn(samps, seqs_t, "Latitude", Latitude)
  Latitude_prop_asv[[Latitude]] <- sub
}
# find intersecting ASVs for all countries
common_asvs <- Reduce(intersect, Latitude_prop_asv)
...
# create list of assigned species per Latitude
Latitude_prop_specs <- list()
for (Latitude in samps$Latitude) {
  sub <- find_specs_for_venn(samps, species_df, "Latitude", Latitude)
  Latitude_prop_specs[[Latitude]] <- sub
}
# find intersecting species for all countries
common_specs <- Reduce(intersect, Latitude_prop_specs)
...
most_pres_Latitude <- capture.output(as.data.frame(table(samps_bd_most_pres$Latitude)))
...
most_abun_Latitude <- capture.output(table(samps_bd_most_abun$Latitude))
...
samps$Latitude <- as.character(samps$Latitude)
samps$Latitude[is.na(samps$Latitude)] <- "NA"
table(samps$Latitude)
