...
REV <- "CAHCGATGAAGAACGCAG"#"CGCGCGCACGTTTCAHCGATGAAGAACGCAG"
FWD <- "CCTCCSCTTANTDATATGC"#"GCATATHANTAAGSGGAGGTGACTGGCCGCCT"


### store base name of fastq files that you don't want in sample names
base_prefix <- ""
...
run <- "China_Mongolia_SouthKorea_Taiwan_2018/"
country <- "PosC_test/"
...
path <- paste0(root_path, run, "DADA2_Results/", country, "Original_Data/", "Sample_Seqs/")
path2 <- paste0(root_path, run, "DADA2_Results/", country, "Original_Data/", "Sample_Seqs/filtN/")
path_out <- paste0(root_path, run, "DADA2_Results/", country, "HPC_Outputs/")

unite.ref <- "/Users/lucy/Documents/MRes/MycobiomeProject/Analysis/UNITE_database/sh_general_release_dynamic_s_04.02.2020.fasta"
...
# check and adjust orientation of primers to ensure orientation matches samples
for (i in 1:length(fnFs.filtN)) {
  # view orientation of primers for sample i (wil be the same for all samples)
...
cutadapt <- "/Users/lucy/Library/Python/3.7/bin/cutadapt"
...
for (i in 1:length(fnFs.filtN)) {
  primer_check <- rbind(FWD.ForwardReads = sapply(FWD.orients, primerHits, fn = fnFs.cut[[i]]),
  ...
  print(paste0("Primer count for: ", i))
  print(primer_check)
}
...
pdf(file = paste0(path_out,"Quality_Profiles_diffprimer.pdf"), paper = 'A4', onefile = TRUE)
...
# - enforce minimum length of 50bp to remove spurious very low length sequences
...
# transpose abundances dataframe
...
write.table(seqtab.slim,paste(path_out,"Abun_Table_tax_row.txt",sep = ''))
